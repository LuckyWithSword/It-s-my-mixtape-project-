  ...
  const handleFinalCreate = async (skipArtwork: boolean = false) => {
    ...
    try {
      ...
      if (pendingOptimizedImage && !skipArtwork) {
        if (!isCloudinaryConfigured() && typeof window !== 'undefined' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
          // Graceful fallback: when Cloudinary build variables have not been configured on Netlify,
          // do not crash the publish pipeline. Continue saving the mixtape with retro cassette artwork.
          console.warn('[CLOUDINARY] Cloudinary build variables not set in Netlify; continuing save with cassette artwork.');
          finalImageUrl = null;
        } else {
